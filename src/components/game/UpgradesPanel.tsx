...
export default function UpgradesPanel({ gameState, onBuyUpgrade, playPurchase }: UpgradesPanelProps) {
  ...
  const renderUpgradeButton = (upgrade: Upgrade) => {
    ...
    return (
      ...
      >
        ...
        <div className="text-xs">Cost: {cost} clicks</div>
        <div className="text-xs">Effect: +{upgrade.effect} {upgrade.type === 'clickPower' ? 'click power' : 'auto-clickers'} per owned</div>
      </button>
    );
  };
  ...
  return (
    <div className="p-4">
      <h3 className="font-bold mb-2">Click Power Upgrades</h3>
      {clickPowerUpgrades.map(renderUpgradeButton)}

      <h3 className="font-bold mt-4 mb-2">Auto-Clickers</h3>
