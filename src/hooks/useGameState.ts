...
export function useGameState() {
  ...
  const buyTranscendenceNode = useCallback((id: string) => {
    setGameState(prev => {
      ...
      const newState = { ...prev, transcendenceTree: newTree, transcendencePoints: prev.transcendencePoints - node.cost };
      return { ...newState, clickPower: calculateClickPower(newState), cps: calculateCPS(newState) };
    });
  }, []);
      ...
      const newQuests = prev.questState.quests.map(quest => {
        ...
        const updatedSteps = quest.steps.map(step => {
          ...
          switch (step.type) {
            ...
            case 'ascensions': current = prev.totalAscensions; break;
            case 'transcendences': current = prev.totalTranscendences; break;
          }
          return { ...step, current };
        });
  ...
  }, [gameState.clicks, gameState.lifetimeClicks, gameState.cps, gameState.totalPrestiges, gameState.totalAscensions, gameState.totalTranscendences, gameState.clickPower, gameState.upgrades]);
  ...
  }, [gameState.lifetimeClicks, gameState.totalPrestiges, gameState.totalAscensions, gameState.totalTranscendences, gameState.cps, gameState.upgrades]);
