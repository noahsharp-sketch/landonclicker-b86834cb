...
interface PrestigePanelProps {
  ...
  onPrestige: () => void;
  onAscend: () => void;
  onBuySkillNode: (id: string) => void; // original function
  onBuyAscensionNode: (id: string) => void; // original function
...
export function PrestigePanel({
  ...
  const SkillNodeButton = ({ node }: { node: SkillNode }) => {
    ...
    return (
      <button
        onClick={() => canAfford && onBuySkillNode(node.id)} // <-- original function
        disabled={!canAfford}
  ...
  const AscensionNodeButton = ({ node }: { node: AscensionNode }) => {
    ...
    return (
      <button
        onClick={() => canAfford && onBuyAscensionNode(node.id)} // <-- original function
        disabled={!canAfford}
        className={`w-14 h-14 rounded-lg flex flex-col items-center justify-center font-bold text-xs transition-all
