...
export function useGameState() {
  ...
  const updateEventProgress = useCallback((state: GameState, eventId: string) => {
    ...
    let newState = { ...state };
    newState = updateQuestProgress(newState, 'specialEvent', 1);
    return newState;
  }, [updateQuestProgress]);
  ...
  const buyUpgrade = useCallback((id: string) => {
    setGameState(prev => {
      ...
      if (upgrade.type === 'autoClicker') {
        newState = updateQuestProgress(newState, 'autoClicker', 1);
      }
  ...
  const buyUpgradeBulk = useCallback((id: string, amount: number | "MAX") => {
    setGameState(prev => {
      ...
      if (upgrade.type === 'autoClicker') {
        newState = updateQuestProgress(newState, 'autoClicker', bought);
      }
  ...
  // -----------------------------
  // Prestige / Ascend / Transcend / Eternity with quest tracking
  // -----------------------------
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setGameState(prev => {
        ...
        // Track completed special events
        prev.questState.events.forEach(event => {
          if (event.completed && !event.claimed) {
            newState = updateEventProgress(newState, event.id);
          }
        });
